...
//        Eigen::Vector3f euler_angles = matrix.eulerAngles(2, 1, 0);
//        euler_angles.z() = -euler_angles.z();
//        matrix = euler_angles.matrix();
        return matrix;
    }
...
//                std::cout << "imu unsync time:" << UnsyncedData.front().time << std::endl;
//                std::cout << "imu sync time:" << sync_time << std::endl;
                return false;
            }
...
//            std::cout << "imu size problem" << std::endl;
            return false;
        }
        IMUData front_data = UnsyncedData.at(0);
        IMUData back_data = UnsyncedData.at(1);
        IMUData synced_data;
        synced_data = Interpolation(front_data, back_data, sync_time);

        SyncedData.push_back(synced_data);

        return true;
    }
    IMUData IMUData::Interpolation(IMUData front_data, IMUData back_data, double sync_time) {
